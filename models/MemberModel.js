...
const Member = {
	...
	async getMemberOfTeam(teamID) {
		return databaseConnection.queryp("SELECT User.User_Id, User.Firstname, User.Name, User.Email FROM User INNER JOIN member_of ON member_of.User_Id = User.User_Id WHERE member_of.Team_Id = ?;", [teamID]);
	},
